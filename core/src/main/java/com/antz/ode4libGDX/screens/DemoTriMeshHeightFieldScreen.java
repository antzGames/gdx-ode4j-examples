...
import static com.antz.ode4libGDX.screens.BunnyGeom.IndexCount;
import static com.antz.ode4libGDX.screens.BunnyGeom.Indices;
import static com.antz.ode4libGDX.screens.BunnyGeom.Vertices;
...
public class DemoTriMeshHeightFieldScreen implements Screen, InputProcessor {
    ...
    private ModelInstance m, heightMI;
    private Color color;
    private DQuaternionC qOde;
    ...
    // some constants
    private static final int NUM = 3;			// max number of objects
...
//        scene.render(); // render Mundus scene
    ...
    public void doStep (boolean pause) {
        ...
        // Draw Heightfield
        drawGeom(gheight, -1,null, null, false);
        ...
        for (int i=0; i<num; i++) {
            for (int j=0; j < GPB; j++) {
                ...
                drawGeom (obj[i].geom[j], i,null, null, show_aabb);
            }
        }
    }

    // draw a geom
    private void drawGeom (DGeom g, int objInstance,DVector3C pos,  DMatrix3C R, boolean show_aabb) {
        if (g==null) return;
